...
author: "Orlando Acevedo-Charry, José M. Ponciano, Caroline L. Poli, Brian M. Jeffery, Robert J. Fletcher Jr., María Ángela Echeverry-Galvis, Bette A. Loiselle, Scott K. Robinson, and Miguel A. Acevedo"
...
| ![Graphical abstract for Monitoring population extinction risk with community science data](results/Graphical Abstract.png){width=100%}<br>

# Read me (begin here)

This file is a tutorial for the paper in Journal of Applied Ecology by [Acevedo-Charry *et al.*, 2025](10.1111/1365-2664.70108). We hope this code serves as a practical tutorial and is applied for different users in an intuitive way. Our aim is to describe and test a quantitative approach that fits continuous state-space models iteratively to a time series of community science data ([eBird](https://ebird.org/home)), with the ultimate goal of estimating local persistence probability through time. We evaluated model accuracy by comparing estimates and trends using eBird data with those estimated from the endangered Everglade’s snail kite long-term standardized monitoring project. We also performed a sensitivity analysis to assess how robust the persistence estimates are to a reduction in the number of eBird observations available. We used the risk-based viable population monitoring (VPM) framework ([Staples *et al.*, 2005](https://conbio.onlinelibrary.wiley.com/doi/full/10.1111/j.1523-1739.2005.00283.x)), fitting continuous state-space population models under density-independent dynamics ([Humbert *et al.*, 2009](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x)). 

This population dynamic model can also be extended to density-dependent dynamics ([Dennis & Ponciano, 2014](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1); code provided). We focused on the recent (since 2018) expanded population of Everglade's snail kite ([*Rostrhamus sociabilis plumbeus*](https://birdsoftheworld.org/bow/species/snakit/cur/introduction)) in north-central Florida, US, a species with standardized monitoring efforts that provide benchmark population trends for comparison (see Section 2.1 in the main text).  
...
time $t$, $a = \ln{\lambda}$ is a constant of
...
$$
\ln{L}(x_0, a, \sigma^2, \tau^2) = -\frac{(q+1)}{2}\ln(2\pi)-\frac{1}{2}\ln(|\mathbf{V}|)-\frac{1}{2}(\mathbf{y}-\mathbf{m})'\mathbf{V}^{-1}(\mathbf{y}-\mathbf{m})
$$
...
guess_egss <- function(yt,tt){
  ...
  Tbar      <- mean(t.i)
  # trend parameter for EGOE (a = ln(lambda) in H_O)
  a.egoe    <- sum((t.i-Tbar)*(yt-Ybar))/sum((t.i-Tbar)*(t.i-Tbar));
  # Initial population  of EGOE
  x0.egoe   <- Ybar-a.egoe*Tbar
  # sigma square for EGOE is 0 (assume no ecological process variation)
  ssq.egoe  <- 0
  # estimate of initial population observed under EGOE
  Yhat.egoe <- x0.egoe+a.egoe*t.i;
  ...
  a.egpn    <- sum(Ttr*Ytr)/sum(Ttr*Ttr);
...
```{r load-negloglike_egss_reml}
negloglike_egss_reml <- function(fguess,yt,tt){

  sigmasq <- exp(fguess[1]); #in egss_reml, this have only the two parameters
  tausq   <- exp(fguess[2]);
  q       <- length(yt) - 1;
  qp1     <- q+1;
...
To compute the EGSS-REMLs we use the function `egss_reml()`

```{r load-egss_reml}
egss_reml <- function(yt,tt,fguess){
  ...
  #to estimate trend parameter (a) and initial population (x0)
  vx      <- matrix(0,qp1,qp1);
  for(i in 1:q){
  ...
  #Trend parameter
  a.reml=(t(j1)%*%V1inv%*%W.t)/(t(j1)%*%V1inv%*%j1);

  j=matrix(1,qp1,1);
  Vinv=ginv(V);

  #initial population
  x0.reml=(t(j)%*%Vinv%*%(yt-as.numeric(a.reml)*t.i))/(t(j)%*%Vinv%*%j);

  #Extract REMLs and AIC
  remls      <- c(a.reml,exp(optim.out$par[1:2]),x0.reml)
  lnL.hat     <- - optim.out$value[1]
  AIC         <- -2*lnL.hat + 2*2 #where 2 = length(REMLs)...

  out         <- list(remls=remls,
                      lnL.hat = lnL.hat,
                      AIC=AIC)
  return(out)
}
...
With the EGSS-REML values, we can predict the trajectory of the latent ecological process with the function `egss_predict()`
...
egss_predict <- function(yt,tt,parms,plot.it="TRUE"){
  ...
  m[1]=x0.reml; # Initial mean of Y(t).
  ...
  { # using Kalman filter (see equations 6 & 7, # Dennis et al. (2006)).
    m[ti+1]=a.reml+(m[ti]+((v[ti]-tausq)/v[ti])*(yt[ti]-m[ti]));
...
And we can conduct a parametric bootstrap to estimate confidence intervals of the predicted trajectory

```{r load-egss_pboot}
egss_pboot <- function(B=2,parms,yt, tt){
	...
	preds.boot1<- matrix(0,nrow=B,ncol=length(t.i))
		...
		all.sims  <- egss_sim(nsims=B,parms=parms,tt=tt);
		all.preds <- egss_predict(parms=parms, yt=yt,tt=tt,plot.it="FALSE")
		reml.preds<- all.preds[[1]][,2]

		for(b in 1:B ){
		
			bth.timeseries <- all.sims[,b];
			remles.out <- egss_reml(yt=bth.timeseries, tt=tt, fguess=parms);
			boot.remles[b,] <- c(remles.out$remls, remles.out$lnL.hat);
			all.bootpreds <- egss_predict(parms=remles.out$remls, yt=bth.timeseries,tt=tt,plot.it="FALSE");
		...
		colnames(CIs.mat) <- c("a", "sigmasq","tausq", "x0");
		...
		preds.CIs1 <- t(rbind(tt,reml.preds-(mean.boots-preds.CIs1[1,]), reml.preds, reml.preds+(preds.CIs1[2,]-mean.boots)));
		colnames(preds.CIs1) <- c("Time","CI.lower","REMLE","CI.upper");
...
To compute the OUSS-REMLs we implement the function `ouss_reml()`
...
ouss_reml <- function(yt, tt, fguess){
  ...
  optim.out     <- optim(par = guess.optim,
                         ...
                         yt=yt,
                         tt=t.i);
  # Restricted maximum likelihood estimates (REMLs) and lnL.hat
...
With the OUSS-REML values, we can predict the trajectory with the
...
| ![Log in your eBird account](data_raw/Sign_in_eBird.png){width=100%}
<br>
...
| ![eBird home - go to the bottom of the site](data_raw/eBird_home_ed.png){width=100%}
<br>
...
| ![eBird data access](data_raw/eBird_access.png){width=100%}
<br>

Then, you can select by species, region, and/or date. In our case, lets
download *Rostrhamus sociabilis* in Florida (US).

| ![Requesting eBird data for snail kites in Florida, US - screenshot of the downloaded data for November 2024](data_raw/SnailKite_Florida_access_eBird.png){width=100%}
<br>
...
| ![Downloaded file - this image represent the version up to June 2024 of a previous run of the method](data_raw/files_deployment.png){width=100%}
<br>
...
| ![First six rows of the ebd_filt file](data_raw/ebd_filt_head.png){width=100%}
<br>
...
| ![First six rows of the ebd_filt file - note new columns were added](data_raw/ebd_filt_head2.png){width=100%}
<br>
...
| ![First six rows of the ebd_filt file - adding cell](data_raw/ebd_filt_head_cell.png){width=100%}
<br>
...
| ![Two areas surveyed by the Snail Kite Project](data_raw/PP_and_PPC_map-min.png){width=100%}
<br>
...
| ![world1](data_raw/world1.png){width=100%}
<br>
...
| ![Head of Snail Kite observations per cell](data_raw/head_obs_SK_cell.png){width=100%}
<br>
...
| ![Head of wrapped grid for Snail kites in Florida](data_raw/head_cells_SK_wrapped.png){width=100%}
<br>
...
```{r fig.cap = "Time series counts per week in two data sets"}
...
Initial dynamic of the population is depicted to the left of the vertical dotted gray line (weeks 1-101; from the first week of January 2018 to the first week of December 2019). Local persistence probability estimation ($\hat{\varphi}$) will correspond to the weeks to the right of the dotted gray line ($n_{eBird} = 258$, $n_{SM}=32$). Red dashed lines represent two threshold values of quasi-extinction for each dataset ($N_{c}^{SM} = 32$, $N_{c}^{eBird} = 5$), one half of the mean observed counts of the entire time series.

\clearpage

# Viability Population Monitoring framework and local persistence estimation - $\hat{\varphi}$
...
The function `egss_reml()` fit an EGSS model and compute the Restricted
...
order) to the trend parameter ($\hat{a}_{eBird}=0.0324$), the
...
```{r predict initial dynamic trajectory with model fitted, fig.cap="EGSS fitted to eBird"}
sk.egss.predict.init <- egss_predict(yt = yt1,
                                tt = tt1,
                                parms = sk.egss.parms$remls,
                                plot.it = TRUE)
...
We can estimate the lower and upper 95% confidence intervals of the estimation with a parametric bootstrap

```{r lower and upper CI}
sk.egss.predict.init.CI <- egss_pboot(B = 100, 
                                  parms = sk.egss.parms$remls,
                                  yt = yt1,
                                  tt = tt1)

plot(x = sk.egss.predict.init.CI$preds.CIs1[,1],
     y = sk.egss.predict.init.CI$preds.CIs1[,3],
     type = "l",col = "black", 
     ylab = "Abundance", xlab = "Time", main = "eBird",
     ylim = c(0,45))
lines(x = sk.egss.predict.init.CI$preds.CIs1[,1],
     y = sk.egss.predict.init.CI$preds.CIs1[,2],
     lty = 2, col = "darkgray")
lines(x = sk.egss.predict.init.CI$preds.CIs1[,1],
     y = sk.egss.predict.init.CI$preds.CIs1[,4],
     lty = 2, col = "darkgray")
points(x = sk.egss.predict.init[[1]][,1],
       y = sk.egss.predict.init[[1]][,3],
       col = "blue")
```
...
skSM.egss.parms <- egss_reml(yt = ytSM1,
                            tt = ttSM1,
...
($\hat{a}_{SM}=0.0489$), the environmental noise
($\hat{\sigma}_{SM}^2=0.0295$), observation error noise
($\hat{\tau}_{SM}^2=0.0268$), and initial population
($\hat{x_0}=0.1311$). 

It is interesting that $\hat{\sigma}_{SM}^2$ is very similar to $\hat{\sigma}_{eBird}^2$ ($\sim 0.03$), as well as $e^{x_0 = 0.1311}_{SM}\approx1$ and $e^{x_0 = -0.0160}_{eBird}\approx1$. Similarly, the expected change in log-abundance per week describe lower change in eBird ($\hat{a}_{eBird}=0.0324$) and higher changes in standardized monitoring weekly counts ($\hat{a}_{SM}=0.0489$). In contrast, the observation error is about an order of magnitude bigger in eBird data with respect to the standardized monitoring ($\hat{\tau}_{SM}^2=0.0268$ $<<$ $\hat{\tau}_{eBird}^2=0.2382$). 

Let's predict the trajectory and change the column names for the
figure.

```{r predict initial dynamic trajectory SK project, fig.cap="EGSS fitted to SM"}
skSM.egss.predict.init <- egss_predict(yt = ytSM1,
                                tt = ttSM1,
                                parms = skSM.egss.parms$remls,
                                plot.it = T)
#change names to combine
colnames(skSM.egss.predict.init[[1]]) <- c("Time.t", "Estimated_SKProj_EGSS","abundance.monitored")
```

We can also compute the CI with parametric bootstrap

```{r lower and upper CI for SM}
skSM.egss.predict.init.CI <- egss_pboot(B = 100, 
                                        parms = skSM.egss.parms$remls,
                                        yt = ytSM1, 
                                        tt = ttSM1)

plot(x = skSM.egss.predict.init.CI$preds.CIs1[,1],
     y = skSM.egss.predict.init.CI$preds.CIs1[,3],
     type = "l",col = "black", 
     ylab = "Abundance", xlab = "Time", main = "Standardized monitoring",
     ylim = c(0,170))
lines(x = skSM.egss.predict.init.CI$preds.CIs1[,1],
     y = skSM.egss.predict.init.CI$preds.CIs1[,2],
     lty = 2, col = "darkgray")
lines(x = skSM.egss.predict.init.CI$preds.CIs1[,1],
     y = skSM.egss.predict.init.CI$preds.CIs1[,4],
     lty = 2, col = "darkgray")
points(x = skSM.egss.predict.init[[1]][,1],
       y = skSM.egss.predict.init[[1]][,3],
       col = "blue")
```
...
```{r Figure 3a, fig.cap="Figure 3a - initial dynamic of the population"}
...
sk.Estimated_figA <- sk.Estimated |>
             ...
             aes(fill = factor(group,
                 ...
                 shape = factor(group,
                           levels = c("abundance.monitored",
                                      "eBird.Observed"))),
               size = 2)+
     scale_fill_manual(values = c("#fc8d5999","#91bfdb99"),
                       labels = c("Standardized Monitoring (SM)", 
                                  "eBird"))+
    scale_shape_manual(values = c(24,21),
                       labels = c("Standardized Monitoring (SM)", 
                                  "eBird"))+
    geom_line(data = sk.Estimated_figA |>
                filter(group %in% c("Estimated_SKProj_EGSS",
                                  "Estimated_eBird_EGSS")),
              aes(color = factor(group,
                           levels = c("Estimated_SKProj_EGSS",
                                      "Estimated_eBird_EGSS"))),
              linetype = "solid", size = 1) +
   scale_color_manual(values = c("#FE996B", "#4575b4"),
                       labels = c("Predicted trajectory (SM - EGSS)", 
...
#       tag = expression(bold("(a)")),
  ...
  scale_x_date(breaks = seq(as.Date("2018-01-01"), 
                            ...
                            by = "3 months"), date_labels="%b \n%Y")+
   theme_classic()+
    geom_segment(aes(x = snailkites.PP$observation.date[92],
                     xend = snailkites.PP$observation.date[96],
                     y = 75,
                     yend = 75),
    ...
    geom_segment(aes(x = snailkites.PP$observation.date[92],
                 ...
                 arrow = arrow(length = unit(0.2, "cm")),
                 color = "darkgray", size = 1)+
  annotate(geom = "text", label = expression(bold(varphi[italic(i)]~" estimation")),
           color = "darkgray",
           x = snailkites.PP$observation.date[93], 
           y = 50, angle = 90, vjust = 1)+
...
```{r Figure 3b, fig.cap="Figure 3b - predicted vs observed trajectories"}
...
FigS3b <- ggplot(sk.Estimated)+
  geom_abline(slope = 1)+
  geom_smooth(aes(x=abundance.monitored, 
                  y=Estimated_SKProj_EGSS),
              method = "lm", color = "#FE996B99", 
              se = F, fill = "#fc8d5905", linetype = "dashed")+
  geom_line(aes(x=abundance.monitored, y=Estimated_SKProj_EGSS), 
             color = "#FE996B", linetype = "solid", size = 1)+
#  geom_point(aes(x=abundance.monitored, y=Estimated_SKProj_EGSS), 
#             color = "black", fill = "#FE996B", shape = 24, size = 2)+
# geom_text(x = 25, y = 50, 
...
#            parse = TRUE, color = "#fc8d59")+
  geom_smooth(aes(x=abundance.monitored, y=Estimated_eBird_EGSS),
              method = "lm", color = "#4575b499", 
              se = F, fill = "#91bfdb05", linetype = "dashed")+
  geom_line(aes(x=abundance.monitored, y=Estimated_eBird_EGSS), 
             color = "#4575b4", linetype = "solid", size = 1)+
#  geom_point(aes(x=abundance.monitored, y=Estimated_eBird_EGSS), 
#             color = "black", fill = "#4575b4", shape = 24, size = 2)+
#  geom_text(x = 50, y = 0, 
#            label = lm_eqn(df = sk.Estimated,
#                           x = sk.Estimated$abundance.monitored, 
...
assume quasi-extinction), in three different periods, given timeframes, or moving simulation windows (\~5 years or $250$ weeks, \~10 years or $500$ weeks, and \~25 years or $1250$ weeks). 

Let assume $N_{critical~eBird} = \frac{1}{2} (e^{\bar{y}}) = 5.47$. The probability 
of persistence in each trajectory $m$ ($\varphi_m$) will be the mathematical complement of the number of time steps within the three different periods (250, 500, 1250) that the population was lower than the threshold. The resulting probability $\varphi_m$ is recorded for each trajectory $m$ ($M = 50,000$), estimating the expected value ($\hat{\varphi}$), while the standard deviation ($\sqrt{Var(\hat{\varphi})}$), first (25%) and third (75%) quartiles as variability of the estimation per each week with data.
...
#Simulation length
len.sim <- c(251, 501, 1251)

#which correspond to three timeframes
timeframes <- c("250 weeks or ~5 years", 
                "500 weeks or ~10 years",
                "1250 weeks or ~25 years")
```


```{r varphi estimation first week, eval=FALSE}
#eval=FALSE
...
for (i in seq_along(len.sim)){
  ...
  phi_mean <- mean(phi)
  phi_SD <- sqrt(var(phi))
  phi_1st <- as.numeric(quantile(phi, probs = 0.25))
  phi_3rd <- as.numeric(quantile(phi, probs = 0.75))
  
  phi_results[[timeframes[i]]] <- cbind(phi_mean,phi_SD, phi_1st, phi_3rd)
  ...
  lines(x = (kde.sims$values*(ntraj/5) + 
    ...
    print(cbind(tt[last.tt], phi_1st, phi_mean, phi_3rd, len.sim[i]-1))
}
...
We can see graphically the trajectories and the difference of $\varphi$ estimated for the week 101. The figures show the observed weekly high counts (blue circles), the fitted model (light blue triangles), 50,000 trajectories (lines; those with
$\varphi<0.5$ in reddish, else in gray), $N_{critical}$ as horizontal dashed
line, and the kernel density estimates of the last point of the
trajectories (reddish below $N_{critical}$, else gray).

| ![First round of trajectories - \~5 years](supporting/FigS2_Step2_1stTrajectories_10_5_250.png){width=100%} <br>

| ![First round of trajectories - \~10 years](supporting/FigS2_Step2_1stTrajectories_10_5_500.png){width=100%} <br>

| ![First round of trajectories - \~25 years](supporting/FigS2_Step2_1stTrajectories_10_5_1250.png){width=100%} <br>

## Add the next observations, repeat model fit, projection, and $\hat{\varphi}$

We can add the next week of observations (week 102; second week of December 2019) to the time-series, re-estimate model parameters as well as the probability of crashing below the $N_{critical}$ during the three timeframes (250, 500, 1250 weeks). Here, users can try to fit the OUSS model the following weeks, and decided by the density dependence parameter $\hat{\theta}$ to fit the EGSS model instead. We provide below the example, but the main results focused on EGSS, which is the expected dynamic in this expanded population.
...
#fit the OUSS
OUSS.partial <- ouss_reml(yt = yt[1:last.tt],
                           tt = tt[1:last.tt],
...
The function `ouss_reml()` fit the OUSS model and estimate parameters. In order, the values correspond to the mean stationary distribution log-abundance ($\hat{\mu}_{eBird}=1.4921$); the trend, speed of equilibration, rate to approach stationary distribution, or density dependence parameter ($\hat{\theta}_{eBird}=4.163928*10^{-7}$); the environmental noise ($\hat{\sigma}^2_{SM}=0.0436$); and the observation error noise ($\hat{\tau}^2_{SM}=0.2339$). Note that the density dependence parameter $\hat{\theta}<0.025$, which suggests density-independence dynamic and the use of the EGSS model instead.

```{r select model and predict week2}
model <- if(OUSS.partial$remls[2] < 0.025){
  "EGSS"}else{
    "OUSS"}
...
EGSS.partial <- egss_reml(yt = yt[1:last.tt], 
                         tt = tt[1:last.tt],
...
$\hat{\varphi}$ values and their standard deviation for three timeframes.
...
| ![Second round of trajectories - \~5 years](supporting/FigS3_Step3_2ndTrajectories_10_5_250.png){width=100%} <br>

| ![Second round of trajectories - \~10 years](supporting/FigS3_Step3_2ndTrajectories_10_5_500.png){width=100%} <br>

| ![Second round of trajectories - \~25 years](supporting/FigS3_Step3_2ndTrajectories_10_5_1250.png){width=100%} <br>

## Iterate the process

### Iterate the process with eBird contrasting OUSS-EGSS (some figures as examples)
...
for (j in seq_along(end_positions)) {
  ...
  png(png_filename, width = 10, height = 5, units = "in", res = 300)
    ...
    if(model == "OUSS"){
      ...
      parms[[j]][[timeframes[i]]] <- OUSS.partial$remls
      
      parcial.predict <- ouss_predict(yt = yt[1:last.tt],
                                      tt = tt[1:last.tt],
                                      parms = OUSS.partial$remls,
                                      plot.it = F)
      ...
      parms[[j]][[timeframes[i]]] <- EGSS.partial$remls
  ...
  print(cbind(l, model, phi_1st, phi_mean, phi_3rd, phi_SD, len.sim[i]-1))
}
...
| ![iterating some weeks](data_raw/IteratingSome.png){width=100%}<br>

#### Trajectories in timeframe of ~5 years.
Let's see the examples.

How is changing in 5 moments $\hat{\varphi}_{eBird\ \sim 5\ years~simulated}$?

| ![week 101 - October 2019 simulated for ~5 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_101_timeframe_250.png){width=100%}<br>

| ![week 211 - January 2022 simulated for ~5 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_250.png){width=100%} <br>
...
| ![week 316 - January 2024 simulated for ~5 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_250.png){width=100%} <br>

#### Trajectories in timeframe of ~10 years.

How is changing in 5 moments $\hat{\varphi}_{eBird\ \sim 10~years~simulated}$?

| ![week 101 - October 2019 simulated for ~10 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_101_timeframe_500.png){width=100%}<br>

| ![week 211 - January 2022 simulated for ~10 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_500.png){width=100%}<br>

The pattern selected on week 211 keeps selecting OUSS, likely misidentifying density-dependence from the eBird data.

| ![week 316 - January 2024 simulated for ~10 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_500.png){width=100%}<br>

#### Trajectories in timeframe of ~25 years.

How is changing in three moments $\hat{\varphi}_{eBird\ \sim 25\ years~simulated}$?

| ![week 101 - December 2019 simulated for ~25 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_101_timeframe_1250.png){width=100%}<br>

| ![week 211 - January 2022 simulated for ~25 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_1250.png){width=100%} <br>

Note that for the week 211 (January 2022) the population model fitted is the density-dependence model (OUSS). However, this might be do to the fact that lower and less variable weekly high counts are similar to the initial population (1), misidentifying density dependence dynamic in this expanding population. We include this example to show the users how to apply the contrast for established populations (see also Section 8 - Other populations of snail kites).

| ![week 316 - January 2024 simulated for ~25 years](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_1250.png){width=100%} <br>

### Iterate each week to extract $\hat{\varphi}$ and SD from eBird contrasting OUSS-EGSS (no figures)

```{r iterate process eBird contrasting OUSS-EGSS, eval=FALSE}
#Change at the end to `{r eval=FALSE}`; it might take long time (~9 hrs)
...
for (j in seq_along(end_positions)) {
  ...
  for (i in seq_along(len.sim)){
    ...
    if(model == "OUSS"){
      ...
      phi_results[[j]][[timeframes[i]]] <- cbind(phi_1st, phi_mean, phi_3rd, phi_SD)
...
| ![Iterating all - init](data_raw/IterateAll_eBird_NoFigs.png){width=100%}<br>

\...

| ![Iterating all - end](data_raw/IterateAll_eBird_NoFigs2.png){width=100%} <br>

This is a longer process due to our time step scale of weeks and simulation window of 25 years ($\sim3$ days; currently is `{r eval=FALSE}`), so we save the `phi_results` (with `phi_1st`, `phi_hat`, `phi_3rd`, and `phi_SD`), and `modelSS` vectors as data frames to make a plot.
...
phi_eBird_df <- do.call(rbind, lapply(seq_along(phi_results), function(j) {
  do.call(rbind, lapply(seq_along(phi_results[[j]]), function(i) {
    data.frame(
      ...
      phi.1st = phi_results[[j]][[i]][1],
      phi.hat = phi_results[[j]][[i]][2],
      phi.3rd = phi_results[[j]][[i]][3],
      SD.phi = phi_results[[j]][[i]][4],
      Model = modelSS[[j]][[i]]
    )
  }))
}))
...
| ![head screenshot of eBird phi estimations](data_raw/eBird_phi_hat_SD_head.png){width=100%}<br>


### Iterate each week to extract $\hat{\varphi}$ and variation from eBird only EGSS (no figures)
...
#Simulation length (1, 2, and 5 generations)
len.sim <- c(251, 501, 1251)
...
| ![Iterating all EGSS - init](data_raw/IterateAll_eBird_NoFigs_EGSS.png){width=100%}<br>

\...

| ![Iterating all EGSS - end](data_raw/IterateAll_eBird_NoFigs_EGSS2.png){width=100%}<br>

This is a longer process ($\sim3$ days; currently is `{r eval=FALSE}`), so we saved the `phi_results` (with `phi_1st`, `phi_hat`, `phi_3rd`, and `phi_SD`), and `modelSS` vectors as data frames to make a plot. We also saved the four parameters in each week `parms` list.
...
| ![head screenshot of eBird phi estimations](data_raw/eBird_phi_hat_SD_EGSS_parms.png){width=100%}<br>
...
#This will take ~4 hours, consider change `{r eval=FALSE}`
...
for (j in seq_along(end_positions)) {
  ...
  png(png_filename, width = 10, height = 5, units = "in", res = 300)
    ...
    OUSS.partial <- ouss_reml(yt = ytSM[1:last.tt],
                               tt = ttSM[1:last.tt],
    ...
    if(model == "OUSS"){
      ...
      EGSS.partial <- egss_reml(yt = ytSM[1:last.tt],
                                 tt = ttSM[1:last.tt],
...
| ![head screenshot of SM phi estimations - init](data_raw/SM_phi_hat_SD_head.png){width=100%}<br>

\...

| ![head screenshot of SM phi estimations - end](data_raw/SM_phi_hat_SD_head2.png){width=100%}<br:
...
phi_SM_df <- do.call(rbind, lapply(seq_along(phi_results), function(j) {
  do.call(rbind, lapply(seq_along(phi_results[[j]]), function(i) {
    data.frame(
      ...
      Timeframe = timeframes[i],
      phi.1stSM = phi_results[[j]][[i]][1],
      phi.hatSM = phi_results[[j]][[i]][2],
      phi.3rdSM = phi_results[[j]][[i]][3],
      SD.phiSM = phi_results[[j]][[i]][4],
      Model = modelSS[[j]][[i]],
      ln.lambda = parms[[j]][[i]][1],
      sigma.sqr = parms[[j]][[i]][2],
      tau.sqr = parms[[j]][[i]][3],
...
How is changing in 2 moments $\hat{\varphi}_{SM\ \sim five~years~simulated}$?

| ![Standardized monitoring - week 211 - January 2022 simulated for ~5 years](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_250.png){width=100%}<br>

| ![Standardized monitoring - week 316 - January 2024 simulated for ~5 years](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_250.png){width=100%}<br>

#### Trajectories in timeframe of ~10 years.

How is changing in 2 moments $\hat{\varphi}_{SM\ \sim ten~years~simulated}$?

| ![Standardized monitoring - week 211 - January 2022 simulated for ~10 years](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_500.png){width=100%}<br>

| ![Standardized monitoring - week 316 - January 2024 simulated for ~10 years](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_500.png){width=100%}<br>


#### Trajectories in timeframe of ~25 years.

How is changing in 2 moments $\hat{\varphi}_{SM\ \sim 25~years~simulated}$?

| ![Standardized monitoring - week 211 - January 2022 simulated for ~25 years](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_1250.png){width=100%}<br>

| ![Standardized monitoring - week 316 - January 2024 simulated for ~25 years](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_1250.png){width=100%}<br>


# Results - $\hat{\varphi}$

Call the data and establish transformation of second Y-axis (supplementary figure to include time series counts and $\varphi$).
...
We can compare the overlap of $\varphi$ estimation. First we combine the `phi.eBird` and `phi.SM` results.
...
## $\hat{\varphi}$ Temporal trend - Figure 4 and extended figures
...
phi_data_plot <- datesPP |> 
  ...
  ungroup() |> 
  pivot_longer(cols = c(phi.1st, phi.1stSM),
               names_to = "group.1st",
               values_to = "phi_1st") |> 
  drop_na(phi_1st) |>
  pivot_longer(cols = c(phi.hat, phi.hatSM),
               names_to = "group",
               values_to = "phi_hat") |> 
  drop_na(phi_hat) |>
  pivot_longer(cols = c(phi.3rd, phi.3rdSM),
               names_to = "group.3rd",
               values_to = "phi_3rd") |> 
  drop_na(phi_3rd) |> 
  pivot_longer(cols = c(SD.phi, SD.phiSM),
               names_to = "group.SD",
               values_to = "phi_SD") |> 
  drop_na(phi_SD)
...
FigOUSS_A <- ggplot(data = phi_data_plot,
       aes(x = observation.date, 
           y = phi_hat, 
    ...
    geom_linerange(aes(ymin = phi_hat-phi_SD,
                       ymax = phi_hat+phi_SD),
                   alpha = 0.15) +
    geom_linerange(aes(ymin = phi_1st,
                       ymax = phi_3rd),
                   alpha = 0.15) +
    ...
    scale_color_manual(values = c("#91bfdb95",
                                "#fc8d5995"),
                     labels = c(expression(italic(varphi)[eBird]),
                                expression(italic(varphi)[SM])))+
...
Figure4a <- ggplot(data = phi_data_plot |>
         filter(dataset != "eBird.OUSS.EGSS"),
       aes(x = observation.date, 
           y = phi_hat, 
           fill = original.dataset, 
           color = original.dataset, 
           shape = original.dataset)) +
    geom_vline(xintercept = snailkites.PP$observation.date[92], 
             color = "gray", linetype = "dotted") +
    geom_linerange(aes(ymin = phi_hat-phi_SD,
                       ymax = phi_hat+phi_SD),
    ...
    scale_shape_manual(values = c(21,24),
                     labels = c(expression(italic(varphi)[eBird]),
                                expression(italic(varphi)[SM]))) +
    scale_fill_manual(values = c("#91bfdb95",
                                "#fc8d5995"),
                     labels = c(expression(italic(varphi)[eBird]),
                                expression(italic(varphi)[SM]))) +
    scale_color_manual(values = c("#91bfdb95",
    ...
    scale_x_date(limits = c(snailkites.PP$observation.date[90],
               ...
               breaks = seq(snailkites.PP$observation.date[90], 
                            ...
                            by = "12 months"), date_labels="%b\n%Y")+
  labs(x = "Observation date",
       y = expression(paste("Persistence (",italic(varphi),")")),
       tag = expression(bold("(a)")),
       title = "Simulation window: 250 weeks or ~5 years",
       subtitle = "(one generation)")  +
    theme_classic() +
...
FigOUSS_B <- ggplot(data = phi_data_plot,
       aes(x = observation.date, 
           y = phi_hat, 
           color = original.dataset)) +
    geom_vline(xintercept = snailkites.PP$observation.date[92], 
...
Figure4b <- ggplot(data = phi_data_plot |>
         filter(dataset != "eBird.OUSS.EGSS"),
       aes(x = observation.date, 
           y = phi_hat, 
           color = original.dataset, 
...
phi_data_plot <- datesPP |> 
  left_join(phi_combined |> 
              filter(Timeframe == "1250 weeks or ~25 years")) |>
  ungroup() |> 
  pivot_longer(cols = c(phi.1st, phi.1stSM),
               names_to = "group.1st",
               values_to = "phi_1st") |> 
...
FigOUSS_C <- ggplot(data = phi_data_plot,
       aes(x = observation.date, 
           y = phi_hat, 
           color = original.dataset)) +
    geom_vline(xintercept = snailkites.PP$observation.date[92], 
  ...
  labs(x = "Observation date",
       ...
       title = "Simulation window: 1250 weeks or ~25 years",
       subtitle = "(five generations)")  +
    theme_classic() +
    theme(legend.title = element_blank(),
        legend.position = "none",
        legend.spacing.y = unit(-0.5, "cm"),
...
Figure4c <- ggplot(data = phi_data_plot |>
         filter(dataset != "eBird.OUSS.EGSS"),
       aes(x = observation.date, 
           y = phi_hat, 
           color = original.dataset, 
...
#lets saved it with good proportions
ggsave("results/Figure4_PLocalPersi_5-10-25yrsSimulated.pdf", 
       plot = Figure4, dpi = 300, width = 5.5, height = 9, units = "in")

ggsave("results/Figure4_PLocalPersi_5-10-25yrsSimulated.png", 
       plot = Figure4, dpi = 300, width = 5.5, height = 9, units = "in")
```

| ![(*Same figure 4 in the main text*). Estimated local persistence probability ($\hat{\phi}$ +/- standard deviation) during ~5 years of monitoring (December 2019 to November 2024) after 50,000 population trajectories simulated for three timeframes (a-c). ](results/Figure4_PLocalPersi_5-10-25yrsSimulated.png){width=100%}<br>

## $\hat{\varphi}$ Correlation and RMSE between datasets and simulated windows

How correlated are the $\hat{\varphi}$?
...
Lower RMSE in $S_{1250}$ indicates that, on average, close persistence estimates between the two datasets. However, a good compromise between high correlation and low RMSE is provided by the window simulation of $S_{500}$. We can plot the Absolute error difference for overlapping weeks of counts.
...
After converting back the $\hat{x_0}$ parameter to individuals ($N_0$), the estimation for the two datasets is the same ($\text{RMSE}_{\hat{x_0}} = 0$). Similarly, lower values of $\text{RMSE}_{\hat{\sigma}^2}=0.0014$ indicate similar environmental noise estimated for the two datasets in weeks of overlap in monitoring. The following parameter with similar estimation is the trend parameter $\text{RMSE}_{\hat{\ln{\lambda}}}=0.0087$. Finally, the parameter of more difference between the datasets is the the observation noise error, $\text{RMSE}_{\hat{\tau}^2}=0.2007$. 
...
#  geom_line(aes(color = group, linetype = group)) +
  ...
  scale_y_continuous(sec.axis = sec_axis(trans = ~./coeff, 
                                         name = expression("eBird " * italic(varphi)))) +
  ...
  scale_linetype_manual(values = c("solid",
                     ...
                     labels = c("eBird weekly high count", 
                                expression("P(local persistence) = " * italic(varphi))))+
...
#  geom_line(aes(color = group, linetype = group)) +
  ...
  scale_y_continuous(sec.axis = sec_axis(trans = ~./coeffSM, 
                                         name = expression("Standardized Monitored " * italic(varphi)))) +
...
Fig3cExt <- ggplot(phidata)+
    ...
    labs(x = expression("Standardized Monitored "*italic(varphi)),
         y = expression("eBird "*italic(varphi)),
         tag = expression(bold("(c)")),
...
## Results for simulation window of \~10 years (500 weeks) - higher RMSE and $\rho$
...
## Results for simulation window of \~25 years (1250 weeks)

```{r eBird extended figure 25yrs - a, eval=TRUE}
...
| ![Time series of weekly high counts of Snail kites in Payne's Prairie wetland from eBird (a) and standardized monitoring surveys (b), with local persistence probability ($\phi$) +/- standard deviation estimated for each dataset after 50,000 population trajectories simulated. (c) Local persistence probability extimated from eBird data plotted against local persistence probability estimated from standardized monitored, with standard deviation. Estimation of $\phi$ from 50,000 trajectories in a timeframe of \~25 years (1250 weeks). Solid black line is the line of identity, while dashed black line is linear regression (equation in gray).](results/Figure3_timeseries_PLocalPersi150weeks.png){width=100%}<br>
...
As a sensitivity analysis, we randomly removed 5% of the data in the eBird time series for each year, and projected under the simulation window of \~10 years (500 weeks). This process last ~13 hours.
...
for (p in percentages) {
  ...
  for (i in seq_along(end_positions)) {
    ...
    EGSS.partial <- egss_reml(yt = yt_sampled[1:last.tt],
                                 tt = tt_sampled[1:last.tt],
                                 fguess = guess_egss(yt = yt_sampled[1:last.tt],
                                                     tt = tt_sampled[1:last.tt]));
    
    parms[[i]] <- EGSS.partial$remls
    ...
    # Store results in lists for each percentage
     phi_results[[i]] <- cbind(phi_1st, phi_mean, phi_3rd, phi_SD)
...
for (p in names(results)) {
  for (i in seq_along(results[[p]]$phi_sensitivity)) {
    ...
    phi.1st = results[[p]]$phi_sensitivity[[i]][1]
    phi.hat = results[[p]]$phi_sensitivity[[i]][2]
    phi.3rd = results[[p]]$phi_sensitivity[[i]][3]
    SD.phi <- results[[p]]$phi_sensitivity[[i]][4]
...
| ![head and tail of the sensitivity data frame](results/Sensitivity_DF.png){width=100%}<br>
...
```{r correlation in sensitivity, warning=FALSE}
...
sensilab <- c(
  ...
  'p95' = "95% dataset (n = 245; 31)",
  'p90' = "90% dataset (n = 233; 30)",
  'p85' = "85% dataset (n = 218; 29)",
  'p80' = "80% dataset (n = 209; 26)",
  'p75' = "75% dataset (n = 195; 24)",
  'p70' = "70% dataset (n = 181; 24)",
  'p65' = "65% dataset (n = 169; 25)",
  'p60' = "60% dataset (n = 154; 20)",
  'p55' = "55% dataset (n = 144; 20)",
  'p50' = "50% dataset (n = 127; 14)",
  'p45' = "45% dataset (n = 112; 14)",
  'p40' = "40% dataset (n = 104; 12)",
  'p35' = "35% dataset (n = 89; 11)",
  'p30' = "30% dataset (n = 79; 12)",
  'p25' = "25% dataset (n = 66; 7)",
  'p20' = "20% dataset (n = 50; 4)",
  'p15' = "15% dataset (n = 38; 4)",
  'p10' = "10% dataset (n = 26; 2)",
  'p5' = "5% dataset (n = 13; 2)"
)
...
Fig5Ext_a <- ggplot(cor_sen,
  ...
  labs(x = expression(varphi[SM]),
       y = expression(varphi[eBird]),
       title = expression("Persistence probability ("~varphi~") comparison"))+
  scale_x_continuous(limits = c(0,1))+
  scale_y_continuous(limits = c(0,1))+
  coord_equal(ratio = 1)+
  theme_bw() +
...
| ![*Extended figure for Figure 5 in Main text*. Relationship of estimated local persistence probability ($\hat{\varphi}$) from eBird (ordinate) and standardized monitoring (abscissa) under data reduction as part of a sensitivity analysis. The sequence of panels represent reduction from the complete eBird dataset (relationship of trends in Figure 4c) to different percentages of randomly subsampled data, from top-left to bottom-right ($n$ in parenthesis indicates the number of weeks from eBird reduced data and overlapping benchmark weeks). Local persistence probability was estimated using a ten-year moving window of population simulations from iteratively fitted population models. A linear model is fitted and deployed with a blue line, including confidence intervals in gray ribbon ($\rho$ below the equation shows Pearson correlation coefficient); the diagonal line is the line of identity (relation 1:1). See below specific trends of $\hat{\varphi}$ across the time series. Although model accuracy varies with decreasing data and some overestimation of $\hat{\varphi}$ from eBird data suggest less conservative estimation, the relationship with standardized monitoring data might stands with as little as 5% of the observed eBird weekly high counts (about three weeks per year).](results/Figure5ext_a_Sensitivity_correlations.png){width=100%}<br>
...
Fig5Ext_b <- ggplot(data = phiSD_df_sensitivity,
                    aes(x = observation.date))+
  facet_wrap(~factor(Percentage,
                     levels = c("All","p95","p90","p85","p80",
                                "p75","p70","p65","p60","p55",
      ...
      geom_vline(xintercept = snailkites.PP$observation.date[92], 
             color = "gray", linetype = "dotted") +
    geom_linerange(aes(ymin = phi.hat-SD.phi,
                       ymax = phi.hat+SD.phi,
                      y = phi.hat),
                   alpha = 0.15, color = "#91bfdb95") +
    geom_linerange(aes(ymin = phi.1st,
                       ymax = phi.3rd,
                      y = phi.hat),
                   alpha = 0.15, size = 1, color = "#91bfdb95") +
    geom_point(aes(y = phi.hat),
               alpha = 0.65, color = "#91bfdb95", shape = 21,
               fill = "#91bfdb95") +
    geom_linerange(data = phi.SM.500,
                   aes(y = phi.hatSM,
                       ymin = phi.hatSM-SD.phiSM,
                       ymax = phi.hatSM+SD.phiSM),
                   alpha = 0.15, color = "#fc8d5995") +
    geom_linerange(data = phi.SM.500,
                   aes(y = phi.hatSM, 
                       ymin = phi.1stSM,
                       ymax = phi.3rdSM),
                   alpha = 0.15, size = 1, color = "#fc8d5995") +
    geom_point(data = phi.SM.500,
               aes(y = phi.hatSM),
                alpha = 0.65, color = "#fc8d5995", shape = 24,
               fill = "#fc8d5995") +
    scale_y_continuous(expand = c(0,0))+
   labs(x = "Observation date",
       y = expression(paste("Persistence (",italic(varphi),")")))  +
...
ggsave("results/Figure5ext_b_sensitivity_datareduction.pdf", 
       plot = Fig5Ext_b, dpi = 300, width = 10, height = 6, units = "in")
...
### Root Mean Square Error for each list in the reduced dataset
...
Most reduced datasets have low RMSE (<0.1), which indicates that, on average, the close persistence estimates between the Standardized Monitoring and eBird is maintained even with observation reduced data. We can plot the Absolute error difference for overlapping weeks of counts.
...
Another approach to conduct a sensitivity analysis is thinning at the list level. We randomly removed 5% of the data in eBird for each year, and projected under the simulation window of \~10 years (500 weeks). This process last ~13 hours.

```{r Sensitivity2 at the list level, eval=FALSE}
...
for (p in percentages) {
  ...
  for (i in seq_along(end_positions)) {
    ...
    # only for ~ten years window simulation
    # Using EGSS
    ...
    parms[[i]] <- EGSS.partial$remls
      ...
      phi_results[[i]] <- cbind(phi_mean,phi_SD,phi_1st,phi_3rd)
  ...
  # Store results for this percentage
  results.S2[[paste0("p", round(p*100))]] <- list(
    Time.t = tt_sampled[end_positions],
    phi_sensitivity = phi_results,
    parameters = parms,
    time_taken = timelast
...
```{r explorting sensitivity results - list level in df, eval=FALSE}
# Initialize an empty data frame to store results
phiSD_df_sensitivity.S2 <- data.frame(
  Percentage = numeric(),
  Time.t = numeric(),
  phi.1st = numeric(),
  phi.hat = numeric(),
...
# Loop through each percentage in the results
for (p in names(results.S2)) {
  for (i in seq_along(results.S2[[p]]$phi_sensitivity)) {
    # Extract data for this specific percentage p and end position = week i
    phi.1st = results.S2[[p]]$phi_sensitivity[[i]][1],
    phi.hat = results.S2[[p]]$phi_sensitivity[[i]][2],
    phi.3rd = results.S2[[p]]$phi_sensitivity[[i]][3],
    SD.phi <- results.S2[[p]]$phi_sensitivity[[i]][4]
    time_t <- results.S2[[p]]$Time.t[i]
    time_taken <- results.S2[[p]]$time_taken
    ln.lambda <- results.S2[[p]]$parameters[[i]][1]
    sigma.sqr <- results.S2[[p]]$parameters[[i]][2]
    tau.sqr <- results.S2[[p]]$parameters[[i]][3]
    x0 <- results.S2[[p]]$parameters[[i]][4]
    ...
    # Bind the temp_df to the final data frame
    phiSD_df_sensitivity.S2 <- rbind(phiSD_df_sensitivity.S2, temp_df)
  }
}

head(phiSD_df_sensitivity.S2)
tail(phiSD_df_sensitivity.S2)

# Save the data frame to a file
saveRDS(phiSD_df_sensitivity.S2, "results/phi_SD_SensitivityS2_model_parameters.rds")
```

With the data saved, we can compare how the reduction of data affect the inference. 

```{r load data phi and sensitivity2, eval=TRUE}
#Load data saved
  #data organized
  datesPP <- readRDS("data_tmp/datesTimeseriesPaynesPrairie.rds")
  #local persistence from Standardized Monitored
  ...
  phi.eBird.500 <- readRDS("results/phi_hat_SD_eBird_model_parameters.rds") |>
    ...
    filter(Timeframe == "500 weeks or ~10 years")
  #data reduced
  phiSD_df_sensitivity.S2 <- readRDS("results/phi_SD_SensitivityS2_model_parameters.rds") |>
    left_join(datesPP) |>
    full_join(phi.eBird.500 |>
                mutate(Percentage = "All",
                       x0 = round(exp(x0))) |>
...
```{r correlation in sensitivity2, warning=FALSE, eval=TRUE}
#generate a dataframe to estimate correlation and make a figure
cor_sen2 <- phiSD_df_sensitivity.S2 |>
  left_join(phi.SM.500 |>
              mutate(x0 = round(exp(x0))) |>
              rename(Trend = ln.lambda,
                       Env.noise = sigma.sqr,
...
Fig5Ext_a2 <- ggplot(cor_sen2,
  ...
  labs(x = expression(varphi[SM]),
       y = expression(varphi[eBird]),
       title = expression("Persistence probability ("~varphi~") 2nd comparison"))+
  scale_x_continuous(limits = c(0,1))+
  scale_y_continuous(limits = c(0,1))+
  coord_equal(ratio = 1)+
  theme_bw() +
...
#lets saved it with good proportions
ggsave("results/Figure5ext_a_Sensitivity2_correlations.pdf", 
       plot = Fig5Ext_a2, dpi = 300, width = 9, height = 8, units = "in")

ggsave("results/Figure5ext_a_Sensitivity2_correlations.png", 
       plot = Fig5Ext_a2, dpi = 300, width = 9, height = 8, units = "in")
```

| ![Second sensitivity analysis after review. Relationship of estimated local persistence probability ($\hat{\varphi}$) from eBird (ordinate) and standardized monitoring (abscissa) under data reduction as part of a second sensitivity analysis. The sequence of panels represent reduction from the complete eBird dataset (relationship of trends in Figure 4c) to different percentages of randomly subsampled data at the list level, from top-left to bottom-right ($n$ in parenthesis indicates the number of weeks from eBird reduced data and overlapping benchmark weeks). Local persistence probability was estimated using a ten-year moving window of population simulations from iteratively fitted population models. A linear model is fitted and deployed with a blue line, including confidence intervals in gray ribbon ($\rho$ below the equation shows Pearson correlation coefficient); the diagonal line is the line of identity (relation 1:1). See below specific trends of $\hat{\varphi}$ across the time series. Although model accuracy varies with decreasing data and some overestimation of $\hat{\varphi}$ from eBird data suggest less conservative estimation, the relationship with standardized monitoring data might stands with as little as 5% of the eBird data.](results/Figure5ext_a_Sensitivity2_correlations.png){width=100%}<br>
...
```{r Figure sensitivity2 extended, eval=TRUE}

Fig5Ext_b2 <- ggplot(data = phiSD_df_sensitivity.S2,
         aes(x = observation.date))+
  facet_wrap(~factor(Percentage,
                     levels = c("All","p95","p90","p85","p80",
                                "p75","p70","p65","p60","p55",
    ...
    geom_linerange(aes(y = phi.hat, 
                   ...
                   alpha = 0.15, size = 1, color = "#91bfdb95") +
    geom_point(aes(y = phi.hat),
               alpha = 0.65, color = "#91bfdb95", fill =  "#91bfdb95",shape = 21) +
    geom_linerange(data = phi.SM.500,
                   aes(ymin = phi.hatSM-SD.phiSM,
                       ymax = phi.hatSM+SD.phiSM),
                   alpha = 0.15, color = "#fc8d5995") +
    ...
    geom_point(data = phi.SM.500,
               aes(y = phi.hatSM),
                alpha = 0.65, color = "#fc8d5995", fill = "#fc8d5995", shape = 24) +
    scale_y_continuous(expand = c(0,0))+
   labs(x = "Observation date",
       y = expression(paste("Persistence (",italic(varphi),")")))  +
    coord_cartesian(ylim = c(0,1))+
...
#lets saved it with good proportions
ggsave("results/Figure5ext_b_sensitivity2_datareduction.pdf", 
       plot = Fig5Ext_b2, dpi = 300, width = 10, height = 7, units = "in")

ggsave("results/Figure5ext_b_sensitivity2_datareduction.png", 
       plot = Fig5Ext_b2, dpi = 300, width = 10, height = 7, units = "in")

```

| ![Second sensitivty analysis. Data reduction as sensitivity analysis. Estimation of local persistence probability ($\varphi$) for standardized monitoring (orange) is replicated in each panel. Sequence of panels represent reduction from the complete dataset (same as Figure 4b *in the Main text*) to different percentages of randomly subsampled data, from top-left to bottom-right. ](results/Figure5ext_b_sensitivity2_datareduction.png){width=100%}<br>


### Root Mean Square Error for each list in the (list) reduced dataset

The trends are very similar and include high Pearson correlation ($>0.9$). We calculated the Root Mean Square Error (RMSE) for each percentage to see, on average, how the overlapping weeks differ from each other (lower values indicate close estimate). 

```{r sensitivity2 RMSE phiSM and phieBird, eval=TRUE}
phiSD_df_sensitivity.S2 |> 
  dplyr::select(Time.t,
         Percentage,
         phi.hat) |> 
  left_join(phi.SM.500  |>
  ...
  ggplot(aes(x = factor(Percentage,
    ...
    geom_hline(yintercept = 0.1, 
               linetype = "dashed")+
    labs(x = "Data reduction - list level",
         y = "Root Mean Square Error")+
    theme_classic()
```
...
```{r sensitivit2 temporal absolute error, eval=TRUE}

datesPP |>
  full_join(phiSD_df_sensitivity.S2 |> 
  dplyr::select(Time.t,
         Percentage,
         phi.hat)) |>
  left_join(phi.SM.500  |>
...
Data reduction in time series and correlation for some percentages: 95%, 75%, 50%, 25%, 5%.

```{r Figure 5 with time series and correlations, warning=FALSE}
...
sensilab_fig5 <- c(
  'p95' = expression(bold("(a)")~"95% - temporal (n = 245; 31)"),
  'p75' = expression(bold("(b)")~"75% - temporal (n = 195; 24)"),
  'p50' = expression(bold("(c)")~"50% - temporal (n = 129; 12)"),
  'p25' = expression(bold("(d)")~"25% - temporal (n = 66; 7)"),
  'p5' = expression(bold("(e)")~"5% - temporal (n = 13; 2)")
)

# Filter the data for the required percentages
cor_sen_filtered <- cor_sen[cor_sen$Percentage %in% c("p95",
                                                      "p75",
                                                      "p50",
                                                      "p25",
                                                      "p5"), ]
...
time_series_plot <- function(percentage) {
  ggplot(cor_sen_filtered[cor_sen_filtered$Percentage == percentage, ],
    ...
    geom_linerange(aes(ymin = phi.hat-SD.phi,
             ...
             y = phi.hat),
                   alpha = 0.15, color = "#91bfdb95",
             fill = "#91bfdb95", shape = 21) +
    geom_linerange(aes(ymin = phi.1st,
                       ymax = phi.3rd, 
             y = phi.hat),
                   alpha = 0.15, size = 1, color = "#91bfdb95") +
    geom_point(aes(y = phi.hat),
               alpha = 0.65, color = "#91bfdb95") +
    geom_linerange(data = phi.SM.500,
                   aes(y = phi.hatSM,
                       ymin = phi.hatSM-SD.phiSM,
                       ymax = phi.hatSM+SD.phiSM),
    ...
    geom_point(data = phi.SM.500,
               aes(y = phi.hatSM),
                alpha = 0.65, color = "#fc8d5995",
               fill = "#fc8d5995", shape = 24) +
    scale_x_date(limits = c(datesPP$observation.date[99],
                 ...
                 date_labels="%b\n%Y")+
    labs(x = "Observation date", 
         y = expression(paste("Persistence (", varphi,")"))) +
    theme_classic() +
    theme(legend.position = "none", 
          strip.background = element_blank()) +
...
correlation_plot <- function(percentage) {
  ggplot(cor_sen_filtered[cor_sen_filtered$Percentage == percentage, ],
    ...
    stat_cor(method = "pearson",
             ...
             size = 3, 
             parse = TRUE) +
    labs(x = expression(paste("Persistence (", varphi[SM],")")), 
         y = expression(paste("Persistence (", varphi[eBird],")"))) +
    scale_x_continuous(limits = c(0, 1)) +
    scale_y_continuous(limits = c(0, 1)) +
    coord_equal(ratio = 1) +
    theme_bw() +
...
Figure5 <- (
  (time_series_plot("p95") | correlation_plot("p95")) /
  (time_series_plot("p75") | correlation_plot("p75")) /
  (time_series_plot("p50") | correlation_plot("p50")) /
  (time_series_plot("p25") | correlation_plot("p25")) /
  (time_series_plot("p5") | correlation_plot("p5"))
)

ggsave("results/Figure5_sensitivity_datareduction.pdf", 
       plot = Figure5, dpi = 300, width = 6.5, height = 12.5, units = "in")
ggsave("results/Figure5_sensitivity_datareduction.png", 
       plot = Figure5, dpi = 300, width = 6.5, height = 12.5, units = "in")

```

```{r Figure 5 with time series and correlations2, warning=FALSE, eval=TRUE}

# Define the second sensilab object
sensilab_fig5b <- c(
  'p95' = expression(bold("(f)")~"95% - sampling (n = 258; 32)"),
  'p75' = expression(bold("(g)")~"75% - sampling (n = 258; 32)"),
  'p50' = expression(bold("(h)")~"50% - sampling (n = 254; 31)"),
  'p25' = expression(bold("(i)")~"25% - sampling (n = 242; 32)"),
  'p5' = expression(bold("(j)")~"5% - sampling (n = 149; 24)")
)

# Filter the data for the required percentages
cor_sen_filtered2 <- cor_sen2[cor_sen2$Percentage %in% c("p95",
                                                      "p75",
                                                      "p50",
                                                      "p25",
                                                      "p5"), ]
...
# we can use a function that creates the time series plots
time_series_plot2 <- function(percentage) {
  ggplot(cor_sen_filtered2[cor_sen_filtered2$Percentage == percentage, ],
         aes(x = observation.date)) +
    geom_linerange(aes(ymin = phi.hat-SD.phi,
                       ymax = phi.hat+SD.phi),
                   alpha = 0.15, color = "#91bfdb95") +
    ...
    theme(legend.position = "none", 
          strip.background = element_blank()) +
    ggtitle(sensilab_fig5b[percentage])
}

# and another funtion that creates correlation plots
correlation_plot2 <- function(percentage) {
  ggplot(cor_sen_filtered2[cor_sen_filtered2$Percentage == percentage, ],
         aes(x = phi.hatSM, 
             y = phi.hat)) +
    geom_abline(slope = 1) +
...
# Combine plots using patchwork
Figure5b <- (
  (time_series_plot2("p95") | correlation_plot2("p95")) /
  (time_series_plot2("p75") | correlation_plot2("p75")) /
  (time_series_plot2("p50") | correlation_plot2("p50")) /
  (time_series_plot2("p25") | correlation_plot2("p25")) /
  (time_series_plot2("p5") | correlation_plot2("p5"))
)

ggsave("results/Figure5_sensitivity2_datareduction.pdf", 
       plot = Figure5b, dpi = 300, width = 6.5, height = 12.5, units = "in")
ggsave("results/Figure5_sensitivity2_datareduction.png", 
       plot = Figure5b, dpi = 300, width = 6.5, height = 12.5, units = "in")
```

Combine the figures of the two sensitivity analysis

```{r eval=TRUE, warning=FALSE}

Fig5All <- (Figure5 | Figure5b)

#lets saved it with good proportions
ggsave("results/Figure5_sensitivity_datareductionx2.pdf", 
       plot = Fig5All, dpi = 300, width = 12, height = 13, units = "in")

ggsave("results/Figure5_sensitivity_datareductionx2.png", 
       plot = Fig5All, dpi = 300, width = 12, height = 13, units = "in")

```

| ![*Figure 5 in the Main text*. Data reduction as sensitivity analysis. Estimation of local persistence probability ($\hat{\varphi}$) from standardized monitoring data (orange; $\hat{\varphi}_{SM}$) is replicated in each comparison with estimation of local persistence probability from eBird (blue; $\hat{\varphi}_{eBird}$) under data reduction as temporal (left) and sampling (right) sensitivity analysis. The sequence of panels represent reduction from the complete eBird dataset (same as Figure 4c) to different percentages of randomly sampled data by temporal (a-e; see extended figures in SI) and list-level thinning (f-j; see extended figures in SI). The n in parenthesis indicates the number of weeks from eBird reduced data and overlapping benchmark weeks. Temporal trend of $\hat{\varphi}$ is accompanied with the relationship of $\hat{\varphi}_{eBird}$ (ordinate) and $\hat{\varphi}_{SM}$ (abscissa). A linear model of this relationship is fitted and deployed with a blue line, including confidence intervals in gray ribbon (ρ below the equation shows Pearson correlation coefficient); the diagonal line is the line of identity (1:1). Although model accuracy varies with decreasing data and some overestimation of $\hat{\varphi}_{eBird}$ compared to $\hat{\varphi}_{SM}$ suggests less conservative estimation, the trends are still similar even with as little as 5% eBird data (about three weeks or 75 lists per year). ](results/Figure5_sensitivity_datareductionx2.png){width=100%}<br>


# Aditional example for other populations of snail kite and monthly $\hat{\varphi}$
...
#search for the cells ID with high counts and our interest location
  ...
  # 2702385: Central Everglades1: 25.75671, -80.76607; 
  # 2702386: Central Everglades2: 25.75906, -80.67722; 
  # 2701656: Central Everglades3: 25.76546, -80.79460; 

  # 2689989: West site of Lake Okeechobee1: 26.98044 -81.09575;
  # 2690719: West site of Lake Okeechobee2: 26.99305 -81.06428;
  # 2690722: East site of Lake Okeechobee: 27.12027 -80.67136;

  # 2678323: Northeast Tohopeliga (Toho1): 28.23773 -81.36858;
  # 2678324: East Lake Tohopeliga (Toho2): 28.26645 -81.27500;
  # 2677594: Northcentral Tohopekaliga (Toho3): 28.28354, -81.39557;

  # 2696545: Western Florida1 (Naples): 26.01485, -81.62422
  # 2690713: Western Florida2 (Harns Marsh): 26.64986, -81.68672
  # 2677588: Western Florida3 (Lakeland): 28.07533, -81.94395

sk.others <- SnailKite |>
  filter(cell %in% c(#Central Everglades
                      "2702385","2702386", "2701656", 
                     # Lake Okeechobee
                      "2689989", "2690719","2690722", 
                     # Lake Tohopeliga
                      "2678323","2678324","2677594",
                     # Western Florida
                      "2696545","2690713","2677588")) |> 
  mutate(population = case_when(cell == "2702385"~"Everglades",
                                cell == "2702386"~"Everglades",
                                cell == "2701656"~"Everglades",
                                cell == "2689989"~"Okeechobee",
                                cell == "2690719"~"Okeechobee",
                                cell == "2690722"~"Okeechobee",
                                cell == "2678323"~"Tohopekaliga",
                                cell == "2678324"~"Tohopekaliga",
                                cell == "2677594"~"Tohopekaliga",
                                cell == "2696545"~"Western Florida",
                                cell == "2690713"~"Western Florida",
                                cell == "2677588"~"Western Florida"),
         seqnum = cell,
         cell = case_when(cell == "2702385"~"Everglades 1",
                                cell == "2702386"~"Everglades 2",
                                cell == "2701656"~"Everglades 3",
                                cell == "2689989"~"Okeechobee W1",
                                cell == "2690719"~"Okeechobee W2",
                                cell == "2690722"~"Okeechobee E",
                                cell == "2678323"~"Toho - NE",
                                cell == "2678324"~"Toho - East Lake Toho",
                                cell == "2677594"~"Toho - NC",
                                cell == "2696545"~"WF - Naples",
                                cell == "2690713"~"WF - Harns Marsh",
                                cell == "2677588"~"WF - Lakeland"))
